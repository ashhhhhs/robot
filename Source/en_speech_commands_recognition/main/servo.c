...
void servo_init(void)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        ret = ledc_channel_config(&channels[i]);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "ledc_channel_config failed for channel %d: 0x%x", i, ret);
            return;
        }
    }
...
void servo_set_angle(ledc_channel_t channel, int degrees)
{
    ...
    if (degrees < 0) {
        ESP_LOGD(TAG, "Servo angle below 0: %d, clamping to 0", degrees);
        degrees = 0;
    }
    if (degrees > 180) {
        ESP_LOGD(TAG, "Servo angle above 180: %d, clamping to 180", degrees);
        degrees = 180;
    }
    
    esp_err_t ret = ledc_set_duty(LEDC_LOW_SPEED_MODE, channel, angle_to_duty(degrees));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "ledc_set_duty failed: 0x%x for angle %d on channel %d", ret, degrees, channel);
        return;
    }
    
    ret = ledc_update_duty(LEDC_LOW_SPEED_MODE, channel);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "ledc_update_duty failed: 0x%x for angle %d on channel %d", ret, degrees, channel);
    }
}
